...
NOTE: 默认情况下， Trident Protect 会收集有助于处理您可能提交的任何NetApp支持案例的支持信息，包括集群和受管应用程序的日志、指标和拓扑信息。Trident Protect 会按计划每日向NetApp发送这些支持包。安装Trident Protect 时，您可以选择禁用此支持包集合。您可以手动操作link:trident-protect-generate-support-bundle.html["生成支持包"]随时。
...
----
. 使用 Helm 通过以下命令之一安装Trident Protect。代替 `<name_of_cluster>`集群名称将分配给集群，并用于标识集群的备份和快照：
+
** 正常安装Trident Protect：
+
[source, console]
